...
function App() {
  ...
  const [dashboardActionLists, setDashboardActionLists] = useState<
  ...
  const fetchDashboard = async () => {
    try {
      ...
      setDashboardActionLists(json.actionLists);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const createCompany = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("企業登録エラー:", response.status, errorText);
        throw new Error("企業登録に失敗しました。");
      }
      ...
      // 登録成功後、Dashboard / ActionLists も再取得する
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      setDashboardActionLists(undefined);
    }
  };
